"""The in-container agent program, kept as a single raw string.

Split out of agent_runner.py purely for size (rule 1): this is the code the
executor base64-encodes into the ACI container's command. agent_runner.py
keeps command_for() (the launcher); this module is only the payload. No logic
lives here; editing the agent means editing the SCRIPT text below.
"""
from __future__ import annotations

# Written as a raw string so the executor can base64 it into the ACI `command`
# without a custom image build. Kept terse and dependency-light (openai +
# stdlib only), because it runs inside the stock azure-cli image.
SCRIPT = r'''
